const Hero = () => {
    return (
        <div className="h-lvh flex flex-col gap-4 items-center justify-center">
            <h1 className="text-[max(4vw,48px)] leading-16 p-4 text-balance text-center font-semibold bg-gradient-to-b from-black to-neutral-500 text-transparent bg-clip-text">
                Biodiversity Nature Sustainability Business
            </h1>
            <h2 className="text-xl text-center text-balance">
                For Eco-Friendly Fashion
            </h2>
        </div>
    );
}
 
export default Hero;